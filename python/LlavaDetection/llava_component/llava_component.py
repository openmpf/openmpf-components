...
class LlavaComponent:
    ...
    def get_detections_from_video(self, video_job: mpf.VideoJob) -> Iterable[mpf.VideoTrack]:
        ...
        if video_job.feed_forward_track is None:
            return self._get_frame_detections(video_capture, config, is_video_job=True)
    ...
    def _get_frame_detections(self, media, config, is_video_job=False):
        ...
        self._check_client(config.ollama_client_host_url)

        tracks = []
        for idx, frame in enumerate(media):
            width, height, _ = frame.shape
            detection_properties = dict()
            self._get_ollama_response(self.frame_prompts, frame, detection_properties)
    ...
    def _get_feed_forward_detections(self, job_feed_forward, reader, config, is_video_job=False):
        ...
        if is_video_job:
            for frame, ff_location in zip(reader, job_feed_forward.frame_locations.values()):
                self._get_ollama_response(self.class_prompts[classification], frame, ff_location.detection_properties)
        else:
            self._get_ollama_response(self.class_prompts[classification], reader.get_image(), job_feed_forward.detection_properties)
    ...
    def _get_ollama_response(self, prompt_dict, image, detection_properties):
        try:
