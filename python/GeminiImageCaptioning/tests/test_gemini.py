...
from gemini_component.gemini_component import GeminiComponent, JobConfig
...
class TestGemini(unittest.TestCase):
    def run_patched_job(self, component, job, side_effect_function):
        ...
        with unittest.mock.patch("gemini_component.gemini_component.GeminiComponent._get_model_response", side_effect=side_effect_function):
...
class TestApiBackends(unittest.TestCase):
    def test_openai_response(self):
        component = GeminiComponent()
        config = Mock(
            application_credentials="", base_url="http://vllm:8000/v1",
        ...
        with unittest.mock.patch(
                "gemini_component.gemini_component.OpenAI",
                return_value=client):
            response = component._get_openai_response(
    ...
    def test_google_response(self):
        ...
        with unittest.mock.patch(
                "gemini_component.gemini_component.genai.Client",
                return_value=client):
            response = component._get_google_response(
                config, b"jpeg image bytes", "Describe this image.")
    ...
    def test_json_response_parser(self):
        ...
        self.assertEqual(
            GeminiComponent._parse_json_response(raw_json), expected)
        self.assertEqual(
            GeminiComponent._parse_json_response(
                "```json\n" + raw_json + "\n```"), expected)
