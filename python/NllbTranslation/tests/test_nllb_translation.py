...
"""
        ...
        pt_text_translation = "They fear, in fact, those in whom are indispensable the vivid rays of our languishing sun, or the unclouded face of the moon in the peninsular firmament, where it has not, like that of London--to break at the cost of a plumbeo heaven--to pour joys into the soul and send to the semblances the reflection d'ellas; they imagine fatally pursued from _spleen_, hopelessly gloomy and dreary, as if they were emerging every moment from the subterranean galleries of a pit-coal mine, Our English allies, how they deceive or how they intend to deceive us, is this an illusion or bad faith, against which there is much vain complaint, the lowly and accentuated expression of bliss, which shines on the illuminated faces of the men from beyond the Spot, who seem to walk among us, wrapped in a dense atmosphere of perennial contentment, satisfied with the world, satisfied with men and, most of all, satisfied with themselves."
        ff_track = mpf.GenericTrack(-1, dict(TEXT=pt_text))
        job = mpf.GenericJob('Test Generic', 'test.pdf', test_generic_job_props, {}, ff_track)
        result_track: Sequence[mpf.GenericTrack] = self.component.get_detections_from_generic(job)
    ...
    def test_wtp_with_flores_iso_lookup(self):
        ...
        arz_text_translation = 'There is resentment among some members of the National Assembly for Wales over the proposal to change their functional designation to MWPs. (Members of the Parliament of Wales). This arose from there being plans to change the name of the assembly to the Parliament of Wales.'
