...
class PixtralComponent:
    ...
    def _get_frame_detections(self, job, reader, config):
        ...
        for idx, frame in enumerate(reader):
            ...
            for tag, prompt in self.json_frame_prompts:
                ...
                while (json_attempts <= self.json_limit) and (json_failed):  
                    ...
                    try:
                        ...
                        logger.warning(f"Pixtral failed to produce valid JSON output. Failed {json_attempts} of {self.json_limit} attempts.")
                        continue
    ...
    def _update_detection_properties(self, detection_properties, response_json):
        ...
        if ('PIXTRAL VISIBLE PERSON' not in key_vals) or (key_vals['PIXTRAL VISIBLE PERSON'].strip().lower() not in ignore_words):
