...
class TestAcsTranslation(unittest.TestCase):
        ...
        def validate_results(results):
            try:
                ...
                self.assertEqual(CHINESE_SAMPLE_TEXT_ENG_TRANSLATE,
                                 result.detection_properties['TEXT (TRANSLATED TO EN)'])
    ...
    def test_video_job(self):
        ...
        self.assertEqual(CHINESE_SAMPLE_TEXT_ENG_TRANSLATE,
                         result.frame_locations[0].detection_properties['TEXT (TRANSLATED TO EN)'])
        ...
        self.assertEqual(SPANISH_SAMPLE_TEXT_ENG_TRANSLATE,
                         result.frame_locations[1].detection_properties['TEXT (TRANSLATED TO EN)'])
        self.assertEqual('es',
    ...
    def test_translate_multiple_fields(self):
        ...
        self.assertEqual(SPANISH_SAMPLE_TEXT_ENG_TRANSLATE,
                         result.detection_properties['MORE_TEXT (TRANSLATED TO EN)'])
    ...
    def test_different_to_language(self):
        ...
        result = results[0]
        self.assertEqual(eng_text,
                         result.detection_properties['TEXT'])
        self.assertEqual('Вы говорите по-англиский?',
                         result.detection_properties['TEXT (TRANSLATED TO RU)'])
    ...
    def test_split_text(self, _):
        ...
        self.assertEqual(4, len(detection_props))
        self.assertEqual(text, detection_props['TEXT'])

        expected_translation = (TEST_DATA / 'break-sentence/art-war-translation.txt') \
            .read_text().strip()
        self.assertEqual(expected_translation, detection_props['TEXT (TRANSLATED TO EN)'])
    ...
    def test_category_and_explicit_from_language(self):
        ...
        self.assertEqual(2, len(result.detection_properties))
        self.assertEqual(CHINESE_SAMPLE_TEXT, result.detection_properties['TEXT'])
        self.assertEqual(CHINESE_SAMPLE_TEXT_ENG_TRANSLATE,
                         result.detection_properties['TEXT (TRANSLATED TO EN)'])
    ...
    def test_suggested_from(self):
        ...
        self.assertEqual(input_text, result.detection_properties['TEXT (TRANSLATED TO EN)'])
