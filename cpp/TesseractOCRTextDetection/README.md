...
To get the Tesseract Component to recognized the changed files, it must be restarted.

By default, the component will first check for models in the `MODELS_DIR_PATH` followed
by the default tessdata `TESSDATA_MODELS_SUBDIRECTORY` path. Please ensure that any language models that run together are stored together in the same
