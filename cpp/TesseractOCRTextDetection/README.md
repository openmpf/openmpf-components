...
* An additional 7 parameters are reported in corresponding OCR tracks:
    * PRIMARY_SCRIPT, SECONDARY_SCRIPTS, and ROTATION of text (0, 90, 180, and 270 degrees counterclockwise) in an image.
...
of the primary script score against the second top-detected script score.
    * Similarly, TEXT_ORIENTATION_CONFIDENCE represents the relative confidence of the top text orientation prediction score against the
...
* The second pass will apply another 180 degree rotation on the image before processing.
    * Users can set ROTATE_AND_DETECT_MIN_CONFIDENCE to a positive value to enable confidence checks on the first OCR pass.
...
* To distinguish which track (first pass vs second pass) is returned, the component will also specify the optimal rotation in TWO_PASS_OCR_ROTATION, alongside the original ROTATION property detected by OSD.
