...
using either manual or automated commands.)
...
*  Extract a given model's component files (dictionaries, unicharset, etc.) into target output path:<br/>
   ...
   For example, to extract out the `eng.*-dawg` files from `eng.traineddata` model into another directory: <br/>
       `./tessdata_model_updater -e eng.traineddata extracted_model_dir/eng`

* Convert extracted model `*-dawg` dictionary files to text formatted word list:<br/>
    `./tessdata_model_updater -dw traineddata_unicharset_file traineddata_dawg_file output_text_file`<br/>
    The `unicharset` file can be extracted from an existing `trainedata` model using the `-e` command above.<br/>
    Some language models contain legacy and LSTM `unicharset` files (ex. `eng.unicharset`, `eng.lstm-unicharset`).<br/>
    Please ensure that each `*-dawg` file is paired with the correct `unicharset` file. <br/>
...
* Convert a given wordlist into a model DAWG format:<br/>
    ...
    The `unicharset` file follows the same requirements as the `.*-dawg` to `.txt` conversion step. <br/>
    ...
    to DAWG conversion step.<br/> Subsequent conversation between DAWG and text formats will retain all leftover words.
...
* Combine extracted model components back into a single `*.traineddata` model:<br/>
...
* Automatically update all models in target directory with a given set of new model component files:<br/>
    ...
    For ease of use, users may provide dictionary updates in either `.*-dawg` or `.txt` formats. <br/>
    The given wordlists must match
    the target model component names and file extensions, with text files having an additional `.txt` extension.
...
The following commands will update the `eng.traineddata` file with new dictionary words.
...
Run the following command to extract out wordlists `dawgs` and `unicharset` files:<br/>
`./tessdata_model_updater -e eng.traineddata <TEMP_DIR>/eng`

Run the following command to convert the English word dictionary to text format:<br/>
`./tessdata_model_updater -dw <TEMP_DIR>/eng.unicharset <TEMP_DIR>/eng.word-dawg <TEMP_DIR>/eng.word-dawg.txt`

Run the following command to update the English word dictionary with new words:<br/>
`./tessdata_model_updater -c new-english-wordlist.txt <TEMP_DIR>/eng.word-dawg.txt <TEMP_DIR>/eng.word-dawg-updated.txt`

Run the following command to convert the updated English word dictionary back into the `.word-dawg` file:<br/>
 `./tessdata_model_updater -wd new-english-wordlist.txt <TEMP_DIR>/eng.word-dawg-updated.txt <TEMP_DIR>/eng.word-dawg`

Run the following command to overwrite the component's existing word dictionary with the updated dictionary:<br/>
`./tessdata_model_updater -o eng.traineddata <TEMP_DIR>/eng.word-dawg`

#### Automatic Model Update Commands:
...
-`<UPDATED_MODEL_DIR>` - Will contain the updated `*.traineddata` output models.
    Users can set this equal to `ORIGINAL_TESSDATA_MODEL_DIR` to update all models directly.

Within the `UPDATED_DICT_FILES_DIR`:
- Users can place all language and script model files that they wish to update existing `.traineddata` files with.
- Each updated file must match its target model component by name and extension, unless providing updated word lists.
- For updating word dictionaries, two formatting options are allowed:
    * Providing `[LANGUAGE].*-dawg` in the Tesseract DAWG format. Use the `-dw` conversion option on existing text-based word lists
    to generate these files.
    * Providing `[LANGUAGE].*-dawg.txt` as a text-formatted newline-separated list of words. The word list must match
    the name of the target component with an extra `.txt` extension at the end.
...
- For example, if both `eng.word-dawg.txt` and `eng.unicharset` are in `UPDATED_DICT_FILES_DIR` in the above example commands:
   * Then `eng.unicharset` will first replace the default `unicharset` file for `eng.traineddata`.<br/>
   * Afterwards `eng.word-dawg.txt` will be added to the `eng.traineddata` word dictionary using
     the updated unicharaset file as reference during the wordlist to DAWG conversion step.

Also note that the word list to DAWG conversion step may skip/filter out a small subset of select words
when generating the inital `*-dawg` files. Subsequent conversion between DAWG and text formats will keep remaining words.
