...
* If OSD detects multiple scripts, and MAX_SCRIPTS is >= 2, then OCR will be performed on each detected script given these rules:
    * Each detected script must be >= the MIN_OSD_SCRIPT_CONFIDENCE and MIN_OSD_SCRIPT_SCORE thresholds.
    ...
    * Note that if the number of detected scripts exceeds the MAX_SCRIPTS setting, then only the scripts with the highest scores are considered.
