...
std::map<std::string,std::map<std::string,std::vector<std::string>>> TesseractOCRTextDetection::parse_json(std::string jsonfile_name)
{
    ...
    if (root.find(L"TAGS_STRING_SPLIT") != root.end() && root[L"TAGS_STRING_SPLIT"]->IsObject())
    {
        ...
        LOG4CXX_WARN(hw_logger_, log_print_str( "[" + job_name + "] TAGS_STRING_SPLIT NOT FOUND."));
    }
    ...
    if (root.find(L"TAGS_REGEX") != root.end() && root[L"TAGS_REGEX"]->IsObject())
    {
        ...
        LOG4CXX_WARN(hw_logger_, log_print_str( "[" + job_name + "] TAGS_REGEX NOT FOUND."));
    }
...
bool TesseractOCRTextDetection::get_tesseract_detections(const MPFImageJob &job, std::string &detection, cv::Mat &original, double weight, int psm, std::string lang)
{
    ...
    if( image_data.empty() ) {
        std::cout << "[" << job_name << "] Could not open transformed image and will not return detections" << std::endl;
...
MPFDetectionError TesseractOCRTextDetection::GetDetections(const MPFImageJob &job, std::vector <MPFImageLocation> &locations)
{
    ...
    std::cout << "[" << job_name << "] Processing \"" << job.data_uri << "\"." << std::endl;
    string run_dir = GetRunDirectory();
    if (run_dir.empty()) {
        run_dir = ".";
    }
    ...
    std::cout << "[" << job_name << "] Processing complete. Generated " << locations.size() << " dummy image locations." << std::endl << std::endl;
