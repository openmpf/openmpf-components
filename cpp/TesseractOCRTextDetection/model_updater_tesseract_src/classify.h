...
class Classify : public CCStruct {
 public:
  ...
  // saved in an internal buffer for static model_updater_tesseract_src.
  // Otherwise AdaptToBlob is called for adaption within a document.
  ...
  // Without shape model_updater_tesseract_src, each class_id, config pair represents a single
  // unichar id/font combination, so this function looks up the corresponding
  // font id.
  // With shape model_updater_tesseract_src, each class_id, config pair represents a single
  ...
  // Writes stored model_updater_tesseract_src data to a .tr file based on the given filename.
  ...
  // Parameters.
  // Set during model_updater_tesseract_src (in lang.config) to indicate whether the divisible
