 ...
 * model_updater_tesseract_src-related global parameters.
 *
 * Globals:
...
/**
 * Creates a MasterTrainer and loads the model_updater_tesseract_src data into it:
...
MasterTrainer* LoadTrainingData(int argc, const char* const * argv,
  ...
  *file_prefix = "";
  ...
  // Load model_updater_tesseract_src data from .tr files on the command line.
...
/**
 * This routine reads model_updater_tesseract_src samples from a file and
  ...
  while (fgets(buffer, 2048, file) != nullptr) {
    ...
    if (unicharset != nullptr && !unicharset->contains_unichar(unichar)) {
      ...
      if (unicharset->size() > MAX_NUM_CLASSES) {
        tprintf("Error: Size of unicharset in model_updater_tesseract_src is "
 ...
 * the specified list of model_updater_tesseract_src samples.
